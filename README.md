...
- **Places API (New)**: 地名検索・ピン位置の場所名取得で必要。Androidアプリ制限を有効化したキーを`android/local.properties`に`PLACES_API_KEY`として設定（未設定時は`MAPS_API_KEY`を使用）
- **Routes API**: 経路検索機能で必要。`.env`に`MAPS_API_KEY`として設定
