...
- [x] グループ年表の旅行行に、対象年の旅行を一覧表示する
  - [x] nameがある場合のフォーマット: "name 旅行開始日のyyyy/mm"
  - [x] nameがない場合のフォーマット: "旅行開始日のyyyy/mm"
  - [x] 表示しきれない場合は、途中で省略(…などで省略がわかるように)
  - [x] 行の高さを変更した時に合わせて、表示内容も増減する
