...
- プルリクエストの作成、既存プルリクエストへのpush・本文等の更新、レビューコメントへの返信の前に、`independent-review`スキルに従う。今回の更新差分にドキュメント以外の変更がある場合は独立レビューを必須とし、差分がない場合またはドキュメントのみの場合は不要とする。必要な場合は更新差分だけでなく、プルリクエスト全体と関連経路・影響範囲までレビューする。操作の種類による例外は設けない。
