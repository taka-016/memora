...
- プルリクエストの作成、既存プルリクエストへのpush・本文等の更新、レビューコメントへの返信の前に、`independent-review`スキルに従う。今回の更新差分に実装コード、設定、生成物、テストの変更がある場合は独立レビューを必須とし、差分がない場合またはドキュメントのみの場合は不要とする。操作の種類による例外は設けない。
