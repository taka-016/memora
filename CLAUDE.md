...
1. **必須**: 常に日本語で会話すること
2. **必須**: コメントやテスト名は日本語で記載すること
2. **必須**: ユーザーからの指示に該当する作業が`doc/todo_list.md`に存在しない場合は、**必ず処理を中断**してユーザーに確認すること
3. **必須**: 現在がmainブランチの場合、新しく作業用のブランチを切ること（`CLAUDE.md`の**ブランチ名**に従う）
...
5. **必須**: `./check.sh`でエラーが残っている場合は作業完了としないこと
...
3. **禁止**: `git add .`は使用しないこと。（変更ファイルを確認して今回の作業に関係するファイルのみを追加すること。）
4. **必須**: `CLAUDE.md`の**コミット形式**に従ってコミット&プッシュ&プルリクエストを作成する
5. **必須**: プルリクエストのタイトルと説明に対応する`doc/todo_list.md`の項目を記載する
6. **禁止**: `doc/todo_list.md`のチェックを入れずにコミットすることは絶対に禁止
