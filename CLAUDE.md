...
4. **必須**: 作業の最後に`./check.sh`を実行すること（省略禁止）

### 作業完了時の必須手順
**以下の手順は必ず守ること。この順序を変更したり省略したりしてはならない：**
1. **必須**: 対応する`doc/todo_list.md`の項目に`[x]`チェックを入れる
2. **必須**: 変更ファイルをgit addでステージングに追加する
3. **必須**: `CLAUDE.md`の**コミット形式**に従ってコミットする
4. **禁止**: `doc/todo_list.md`のチェックを入れずにコミットすることは絶対に禁止
