...
- すべての作業はt-wadaのTDDで進めること
- コマンド実行時は&&で繋がず1つずつ実行し、結果を確認しながら進めること
- 作業中に仕様の判断が必要な場合、作業を中断してユーザーに確認すること

### 作業準備手順

1. `./check.sh`を実行し、コードがクリーンな状態であることを確認してから作業を開始する
2. doc/以下の設計資料を確認し、仕様を理解する
3. ユーザーからの指示に該当する作業が`doc/todo.md`に存在しない場合は、**必ず作業を中断**してユーザーに確認する
4. `tree lib test`でディレクトリ構造を確認する
5. 現在がmainブランチの場合、新しく作業用のブランチを切る（`CLAUDE.md`の**ブランチ名**に従う）
...
5. プルリクエストの説明に、作業に対応する`doc/todo.md`の項目を記載する
