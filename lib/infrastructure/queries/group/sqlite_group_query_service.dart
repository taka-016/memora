...
class SqliteGroupQueryService implements GroupQueryService {
  ...
  Future<GroupDto> _group(
    Map<String, Object?> row,
    List<OrderBy>? orderBy,
  ) async {
    final links = await db.rows(
      'group_members',
      where: 'group_id = ?',
      args: [row['id']!],
      orderBy: orderBy,
    ...
    for (final link in links) {
      final member = (await db.rows(
        'members',
        where: 'id = ?',
        args: [link['member_id']!],
      )).single;
      members.add(
        SqliteGroupMemberMapper.fromRow(
          link,
          SqliteMemberMapper.fromRow(member),
        ),
      );
    }
  ...
  Future<List<GroupDto>> getManagedGroupsWithMembersByOwnerId(
    ...
    List<OrderBy>? membersOrderBy,
  }) async => _groups('owner_id = ?', [ownerId], groupsOrderBy, membersOrderBy);
  @override
  Future<List<GroupDto>> getGroupsWithMembersByMemberId(
    ...
    'owner_id = ? OR id IN (SELECT group_id FROM group_members WHERE member_id = ?)',
    [memberId, memberId],
    groupsOrderBy,
    membersOrderBy,
  );
  Future<List<GroupDto>> _groups(
    String where,
    List<Object> args,
    List<OrderBy>? groupsOrderBy,
    List<OrderBy>? membersOrderBy,
    ...
    for (final row in rows) {
      result.add(await _group(row, membersOrderBy));
    }
    return result;
  });
}
