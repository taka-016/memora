...
class Timeline extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final defaultRowSettings = GroupTimelineRowSettingsDto.defaultsForGroup(
      groupWithMembers,
    );
    final defaultRowIds = defaultRowSettings.rows
        .map((row) => row.rowId)
        .toList();
    ...
    final timelineController = useTimelineController(
      ...
      rowIds: defaultRowIds,
      layoutConfig: _layoutConfig,
      onSetRefreshCallback: onSetRefreshCallback,
    );
    final rowDefinitions = buildTimelineRowDefinitions(
      groupWithMembers: groupWithMembers,
      rowSettings: timelineController.rowSettings,
      layoutConfig: _layoutConfig,
    );
    final rowContext = TimelineRowContext(
      groupWithMembers: groupWithMembers,
      tripsByYear: timelineController.tripsByYear,
      dvcPointUsagesByYear: timelineController.dvcPointUsagesByYear,
      groupEventsByYear: timelineController.groupEventsByYear,
      layoutConfig: _layoutConfig,
      rowHeightFor: timelineController.rowHeightFor,
      buildMemberLabels: timelineController.buildMemberLabels,
      saveGroupEvent: timelineController.saveGroupEvent,
    ...
    double rowHeight(TimelineRowDefinition rowDefinition) {
      return timelineController.rowHeightFor(
        rowDefinition.rowId,
        defaultHeight: rowDefinition.initialHeight,
      );
    }
    ...
    Widget buildScrollableDataCells(TimelineRowDefinition rowDefinition) {
      ...
      return Row(
        children: List.generate(columnCount, (columnIndex) {
          ...
          final cellKey = isYearColumn
              ? rowDefinition.cellKeyForYear(year)
              : null;

          return SizedBox(
            width: width,
            height: rowHeight(rowDefinition),
    ...
    Widget buildDataRow(int rowIndex, TimelineRowDefinition rowDefinition) {
      ...
      final height = rowHeight(rowDefinition);
            ...
            children: [
              Material(
                ...
                child: InkWell(
                  ...
                  onTap: rowDefinition.onFixedColumnTap(rowContext),
                  child: Container(
