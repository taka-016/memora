...
- データ取得、表示期間変更、保存・削除、再試行、各Dialog表示の処理中に別操作が割り込んでも、古い計算結果の反映やDialogの重複表示が起きないことを双方向と連続操作で検証する
...
- 行データ取得、再試行、編集Dialog表示、保存・削除、Provider無効化を相互に切り替えた場合も、古い行のDialog表示、更新結果の誤反映、重複操作が起きないことを双方向と連続操作で検証する
