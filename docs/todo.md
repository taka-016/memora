...
- 初回表示では、所属グループとメンバー構成、および旅行、グループイベント、メンバーイベント、DVCポイント利用履歴を取得する
- 自端末で保存・更新・削除した場合は、変更対象が分かっている旅行、グループイベント、対象メンバーのイベント、DVCポイント利用履歴だけを再取得する
- アプリがバックグラウンドから復帰した場合と、ユーザーが年表の手動更新を実行した場合は、外部端末による変更を反映するため、所属グループとメンバー構成、および年表の全行データを再取得する
- 所属グループとメンバー構成、および年表の全行データの再取得は、現在表示中のデータを維持したまま行い、失敗時に既存表示を失わないサイレント更新として実装する
- アプリ復帰が短時間に繰り返された場合は再取得を抑制し、手動更新では抑制せずに最新データを取得する
- 年表データの一括再取得は既存の共通更新経路へ集約し、画面遷移やライフサイクルごとに個別の再取得処理を重複させない
- 旅行の作成・更新・削除が必ず対象Providerの更新通知を通ることを確認し、旅行管理画面から戻った際の年表全行再取得と重複する場合は、保存時の対象別再取得へ統一する
- 初回表示、自端末での対象別更新、アプリ復帰、手動更新、短時間の連続復帰、グループ構成または各行データのサイレント更新失敗、旅行管理画面からの復帰について、期待する再取得範囲と表示維持をテストする
