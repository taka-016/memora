...
- [x] ~~ピンをタップでポップアップメニュー表示~~→仕様変更により廃止
  - [x] ~~「削除」メニューの追加~~
...
- [x] ~~ピンをタップしたときに詳細入力画面をモーダル表示~~→仕様変更によりボトムシート表示に変更
- [x] 詳細入力画面のUIを作成
...
- [x] グループメンバー行の各カラムの2行目に、保育所新生児クラス～大学4年生までの学年を表示する
  - [x] 学年はその年の3月31日時点での年齢で計算する
  - [x] 表示ラベルは「新生児/0歳児/1歳児/2歳児/年少/…/高校3年生/大学4年生」とする
  （生まれた年が新生児で、生まれた日が3/31以前なら新生児/0歳児と併記）
  - [x] 範囲外は空白とする
  - [x] 生年月日が登録されていない場合は空白とする
  - [x] 前行が空白の場合は詰めて表示する
  - [x] 学年の計算ロジックをユースケースに切り出す
- [x] グループメンバー行の各カラムの3行目に、厄年を表示する
  - [x] 厄年は数え年で計算する
  - [x] 数え年は生まれた時が1歳で1月1日（元日）を迎えるごとに1歳年を取る
  - [x] 「前厄/本厄/後厄」まで表示する
  - [x] gender が「その他」や null の場合は空白とする
  - [x] 生年月日が登録されていない場合は空白とする
  - [x] 前行が空白の場合は詰めて表示する
  - [x] 厄年の計算ロジックをユースケースに切り出す
- [x] 画面右上に設定アイコンを表示し、タップで年齢・学年・厄年の表示/非表示の切り替えができるようにする
  - [x] 表示/非表示の設定はローカルストレージに保存し、次回以降も保持する
...
- [ ] 旅行編集画面にタスク管理機能を追加する
  ...
  - [ ] 追加/更新/削除/並び替え/完了状態変更の各操作はユースケース経由のTaskエンティティで処理し、エラーがあった場合は画面上で通知する
