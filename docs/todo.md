...
- [x] 「経路情報」ボタンの追加
  ...
  - [x] GoogleのRoutes APIを使用して経路情報を取得する
  - [x] PinのvisitStartDateの昇順でソートした各Pin間の経路情報を取得する
  - [x] 「経路情報」ボタンから遷移したときの初期の移動手段は自動車(DRIVE)とする
  - [x] 知りたいのは経路なので、出発時間は考慮しない
  - [x] 経路情報ダイアログのUI
    - [x] 各Pin間を下矢印で繋ぎ、矢印の右に経路情報（所要時間、距離、経路概要）を表示する
    - [x] 複数候補がある場合は、タブで切り替えられるようにする
    - [x] 経路情報ダイアログ内で移動手段を切り替えられるようにする。
    - [x] 選択できる移動手段は、自動車(DRIVE)、徒歩(WALK)、公共交通機関(TRANSIT)の3種類
    - [x] 移動手段選択はアイコンボタンで表示する
  - [x] 経路情報ダイアログで移動手段を切り替えた際に古い経路候補を無視する
  - [x] pinのリストが並んでおり、順番をドラッグで入れ替えられる
  - [x] 各pin間はスペースが開いて下矢印でつながり、矢印右に移動手段のプルダウンを表示する
    - [x] プルダウンは自動車(DRIVE)、徒歩(WALK)、公共交通機関(TRANSIT)を選択でき、初期値は自動車(DRIVE)
  - [x] ダイアログ上部に「経路検索」ボタンを配置し、visitStartDateまたは現在時刻を出発時間として各区間ごとにRoutes APIを呼び出す
  - [x] 中間地点を使用せず、pinがn件の場合にn-1回APIを呼び出して区間ごとの経路情報を取得する
  - [x] ダイアログ下半分にGoogleMapを表示し、取得した経路を描画する
    - [x] 各Pinを選択すると該当区間の経路をハイライト表示する
    - [x] map_viewsを使用せず、このダイアログ専用のGoogleMapウィジェットを生成する
    - [x] +/-アイコンでGoogleMapの表示/非表示を切り替えられるようにする
