...
- [x] DVC関連テーブルを実装する
  - [x] dvc_point_contractsテーブルを実装する
    - [x] ER図どおりにカラムを定義する
    - [x] エンティティ・DTO・Mapperを作成する
    - [x] Repository・QueryService・Firestore実装を作成する
  - [x] dvc_limited_pointsテーブルを実装する
    - [x] ER図どおりにカラムを定義する
    - [x] エンティティ・DTO・Mapperを作成する
    - [x] Repository・QueryService・Firestore実装を作成する
  - [x] dvc_point_usagesテーブルを実装する
    - [x] ER図どおりにカラムを定義する
    - [x] エンティティ・DTO・Mapperを作成する
    - [x] Repository・QueryService・Firestore実装を作成する
...
- [x] メンバーのメールアドレスがブランクの場合、メンバー一覧のカードにメールアドレスは表示しない
- [x] メンバー新規登録
...
- [x] 旅行のメモがブランクの場合、旅行一覧のカードにメモは表示しない
...
- [x] 旅行編集画面にタスク管理機能を追加する
  - [x] trip_edit_modalの訪問場所より上部の位置に「タスク管理」ボタンを追加する
  - [x] select_visit_location_viewやroute_info_viewと同様に同一ダイアログ内の表示切り替えで遷移できるようにする
  - [x] TaskViewウィジェットを新設し、expandedSectionに追加して切り替え表示できるようにする
  - [x] タスクはリスト形式で表示し、カード上のチェックボックスでタスクの完了ができるようにする
  - [x] 上部にタスク名の入力欄があり、入力して「追加」ボタンタップするとリストに追加される
  - [x] リストのカード上に削除ボタンを配置する
  - [x] リストのカード上のチェックボックスと削除ボタン以外の箇所をタップでボトムシートが表示され、親タスク、担当者、締切日、メモを入力できる(ボトムシートは別ソースファイルで作成する)
  - [x] 締切日はcustom_date_pickerで入力する
  - [x] 担当者はグループメンバーから選択する
  - [x] 親子タスクの階層は2階層までとするため、既に親タスクがあるタスクは、他のタスクの親タスクとして指定できない
  - [x] 子タスクがあるタスクは親タスクを指定できない
  - [x] 親タスクが指定されている場合、親タスクの下に少しインデント下げる形で表示する(開閉形式で非表示にできる)
...
- [x] タスクの複製を可能にする
  - [x] タスク編集画面にコピーアイコンとペーストアイコンを置く
  - [x] コピー実施で、対象タスクのtripIdを内部保持
  - [x] tripIdを内部保持しているときだけペーストアイコンが使用可能
  - [x] ペースト実施で、tripIdに紐づくタスクを取得し、画面上のタスクと置き換える(すでにタスクがある場合はすべて削除して置き換える)
  - [x] ペースト時にtripIdは現在編集中のtripIdへ更新する
  - [x] タスクのIdは再生成し、親タスクIdもそれに合わせて更新する
  - [x] ペーストは画面値を置き換えることになるため、確認ダイアログを表示する
  - [x] 旅行の新規作成時はコピーアイコン使用不可
...
- [x] pin_detailsを廃止する
- [x] routesを廃止する（ER図、エンティティ、リポジトリなどを整理する）
...
- [x] 旅行編集でメモを空白で更新するとnullになる不具合を修正する（グループのメモと同じく空欄保持）
...
- [x] 編集画面の変更状態をRiverpodで管理し、キャンセル時に確認ダイアログを表示する
  - [x] 状態管理の作成
  - [x] グループ編集画面に適用
  - [x] メンバー編集画面に適用
  - [x] 旅行編集画面に適用
...
- [x] サイドメニューに「DVCポイント計算」を追加する
  - [x] `NavigationItem`にDVCポイント計算を追加する
  - [x] メニュー選択時にDVCポイント計算用の画面フローへ遷移できるようにする
- [x] DVCポイント計算画面の遷移を「グループ一覧 → 計算画面」にする
  - [x] サイドメニューからDVCポイント計算を開いた初期表示はグループ一覧にする
  - [x] グループ一覧でグループを選択すると、選択グループのDVCポイント計算画面を表示する
  - [x] 計算画面の戻る操作でグループ一覧に戻れるようにする
- [x] グループ一覧を年表画面とDVCポイント計算画面で共通化する
  - [x] 年表用のグループ一覧表示ロジックを共通コンポーネント化する
  - [x] 遷移先（年表 / DVCポイント計算）をコールバックで切り替えられる設計にする
  - [x] 共通化後も年表画面の既存挙動を維持する
...
- [ ] DVCポイント計算画面のテストを追加する
  - [ ] 契約登録・契約分離（ユーザーストーリー16）をユニットテストで検証する
  - [ ] ユースケースのポイント計算ロジックをユニットテストで検証する
  - [ ] ポイント消化順（有効期限順）をユニットテストで検証する
  - [ ] 画面遷移（サイドメニュー → グループ一覧 → 計算画面）をウィジェットテストで検証する
  - [ ] 計算画面内の契約登録導線と登録後の表示更新をウィジェットテストで検証する
  - [ ] グループ一覧共通化による年表画面の回帰をウィジェットテストで検証する
