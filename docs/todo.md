...
### 9. オンライン版とオフライン版のAPKを別アプリとして共存させる

- Androidのビルドバリアントをオンライン版とオフライン版に分け、オンライン版は既存のapplication IDを維持し、オフライン版には別のapplication IDを設定する
- 各ビルドバリアントの`MEMORA_APP_MODE`を対応するモードに固定し、Dart、Gradle、Androidネイティブ処理のモードが一致しない構成ではビルドを失敗させる
- オンライン版は既存のFirebase設定を継続して使用し、オフライン版は別application IDでもFirebase設定を要求せずrelease APKをビルドできるようにする
- `./tools/ci/release_android_apk.sh`からオンライン版とオフライン版を判別できるファイル名のrelease APKをそれぞれ生成する
- 生成した2つのrelease APKを同一端末へ同時にインストールしてそれぞれ起動でき、SQLite、SharedPreferences、認証状態、WorkManager、Androidウィジェットの設定とキャッシュがアプリ間で共有されないことを確認する
- 一方のアプリのアンインストールまたはデータ消去が、もう一方のアプリのデータとAndroidウィジェット更新に影響しないことを確認する
- アプリ内でのモード切替、オンライン・オフライン間のデータ移行・同期、Androidウィジェットの名称・外観のモード別変更は行わない
- ビルドバリアント、application ID、モード固定、成果物名を検証するテストを追加し、2つのAPKのビルド・インストール手順をREADMEへ反映する

### 10. 両方のモードを検証して関連資料を更新する
