...
- ユーザーストーリー、業務上の不変条件、利用者から観測できる振る舞い、障害からの復旧を保護するテストを残し、実装詳細や過去の一時的な対策だけを固定するテストを削除する
- `test/unit/main_test.dart`と`test/unit/main_firestore_settings_test.dart`に重複している、テスト内で生成したFirestoreの`Settings`オブジェクトだけを確認するテストを削除し、必要なら実際の起動設定を検証できるテストへ置き換える
- `test/unit/presentation/app/top_page_test.dart`から、モックのスタブ値そのものを確認するテストと、他の画面遷移テストに包含されているテスト環境専用の実Widget表示テストを削除する
- DTOの異なるインスタンスには異なる`hashCode`が必ず割り当てられるという誤った前提のテストを削除する
- Widgetの座標、固定幅、固定高、Widget同士の間隔、改行位置、背景色など、細かな見た目だけを固定するテストを削除する
- Safe Area、長文や多数項目でのオーバーフロー防止、スクロール可能性、操作対象の視認性など、操作不能や表示崩れを防ぐテストは具体的なpx値に依存しない形へ変更する
- DTOとEntityの単純なコンストラクタ代入、`Equatable`自体の動作、同種の`copyWith`確認を整理し、nullable項目の明示的な解除、独自の値変換、Domainの不変条件とバリデーションだけを重点的に残す
- 例外の捕捉や変換を行わない単純なUseCaseから、下位層の例外がそのまま伝播することだけを確認するテストを削除し、引数変換、並び順、複数処理の順序、トランザクションなどUseCase固有の責務を確認するテストを残す
- グループ管理、メンバー管理、旅行管理、DVCポイント管理について、初期取得、更新、再取得、排他制御、失敗状態をNotifierテストへ集約し、Widgetテストは主要操作、表示結果、Dialog・Snackbar、画面遷移の接続確認に絞る
- `test/unit/presentation/app/top_page_test.dart`を通常ナビゲーション、Androidウィジェット起動、戻る操作、認証状態変更などの責務別に分割し、同じ遷移結果を複数経路で重複確認しているテストを統合する
- `pumpAndSettle`を無条件に使用している箇所を見直し、必要なフレームや非同期完了だけを制御してWidgetテストの実行時間と不安定要因を減らす
- `DateTime.now()`へ直接依存するWidgetテストは`AppClock`の固定時刻へ差し替え、月末、年末、実行タイミングに左右されないようにする
- 整理前後で`./check.sh`を実行し、DVC計算、学年・厄年計算、Domainバリデーション、Firestore変換・保存、関連データ削除、非同期競合と再試行、依存方向、Androidウィジェット連携の重要なテストが維持されていることを確認する
