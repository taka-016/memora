...
- [ ] グループ年表のグループイベント行にイベントを表示する
- [ ] 年表セルのタップでフリー入力メモ用のダイアログを開き、グループイベントを追加できるようにする
- [ ] グループイベント専用の管理画面を作成しない構成に合わせて設計資料を更新する
  - [ ] ER図のgroup_events定義をメモ中心の入力仕様に合わせて見直す
  - [ ] エンティティ定義（GroupEventなど）をフリー入力メモ前提の構造に見直す
