...
- ER図のメンバーイベントの変更に伴う対応
  ...
  - member_events用のクエリ処理をER図準拠で見直し、年表表示に必要な単位で取得できる `QueryService` / Firestore実装 / factory配線を追加または改修する
  - member_eventsの取得・保存ユースケースを、上記のリポジトリ契約とクエリ契約に合わせて整理し、Presentation層へ旧フィールド前提を漏らさない構成にする
