...
- [ ] 「経路情報」ボタンの追加
  ...
  - [ ] GoogleのRoutes APIを使用して経路情報を取得する
  - [ ] PinのvisitStartDateの昇順でソートした各Pin間の経路情報を取得する
  - [ ] 「経路情報」ボタンから遷移したときの初期の移動手段は未選択(TRAVEL_MODE_UNSPECIFIED)とする
  - [ ] 知りたいのは経路なので、出発時間は考慮しない
  - [ ] 経路情報ダイアログのUI
    - [ ] 各Pin間を下矢印で繋ぎ、矢印の右に経路情報（所要時間、距離、経路概要）を表示する
    - [ ] 複数候補がある場合は、タブで切り替えられるようにする
    - [ ] 経路情報ダイアログ内で移動手段を切り替えられるようにする。
    - [ ] 選択できる移動手段は、自動車(DRIVE)、徒歩(WALK)、公共交通機関(TRANSIT)、バイク(TWO_WHEELER)、自転車(BICYCLE)の5種類
