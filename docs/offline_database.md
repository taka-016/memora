...
キャッシュ更新はオンライン・オフラインのどちらでも、読み取り開始前に固有の公開世代を取得する。公開世代の読み取り・既存キャッシュの引き継ぎ・現世代の更新は、アプリ内部ストレージのロックファイルを排他的に作成して直列化する。取得結果はその世代専用のファイルへ保存し、後から完了した古い更新は現世代へ書き込まない。対象グループの設定・解除、表示日の変更でも同じ排他処理で世代を進める。取得失敗時は引き継いだキャッシュを表示し続ける。
