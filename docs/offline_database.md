...
キャッシュ更新はオンライン・オフラインのどちらでも、読み取り開始前に固有の公開世代を取得する。取得時点のキャッシュを新しい世代へ引き継いでから表示世代を切り替え、取得結果はその世代専用のファイルへ保存する。後から完了した古い更新は過去世代のファイルだけを書き換えるため、新しい更新結果を上書きしない。対象グループの設定・解除、表示日の変更でも世代を進める。取得失敗時は引き継いだキャッシュを表示し続ける。

Kotlinの描画処理は、現在の公開世代に対応するファイルだけを読み、キャッシュの生成元モード、対象グループ、世代が、現ビルドのモードと現在の選択状態にすべて一致する場合だけ旅行情報を表示する。従来形式のキャッシュはオンラインの第0世代として扱う。
