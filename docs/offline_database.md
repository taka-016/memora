...
手動バックアップ・復元ではDBファイルを直接コピーせず、共有・排他ファイルロックとSQLiteトランザクションを使って論理データを読み書きする。暗号化、SQLite外の状態を含む原子性、復元後同期、Androidの自動バックアップ除外は[オフラインデータの手動バックアップ・復元](offline_backup.md)を参照する。
...
論理バックアップ復元でも、ファイル全体の形式・バージョン・テーブル構成・業務上の不変条件を検証してから書き込む。DB用の`fromRow`をバックアップファイルのパーサーとして使用しない。場所付き入力は復元全体を拒否し、現在のデータを維持する。生SQLによる未知の場所列への書き込みもスキーマで拒否される。
