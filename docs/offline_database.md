...
読み取り前後でSQLiteの`data_version`と接続内の書き込み世代を比較し、DBが更新されていた場合は取得結果を保存せず再取得する。変更がない場合は`BEGIN IMMEDIATE`で保存直前だけ書き込みを止め、再検査してからキャッシュを保存する。これにより、キャッシュ内で変更前後のSQLiteデータが混在することを防ぐ。

キャッシュ更新はオンライン・オフラインのどちらでも、読み取り開始前に固有の公開世代を取得する。公開世代の読み取り・既存キャッシュの引き継ぎ・現世代の更新は、アプリ内部ストレージに置く専用SQLiteファイルの`BEGIN IMMEDIATE`で直列化する。正常終了時はトランザクションをロールバックして接続を閉じ、保持isolateまたはプロセスの異常終了時も接続の終了によってOSがロックを解放する。取得結果はその世代専用のファイルへ保存し、後から完了した古い更新は現世代へ書き込まない。対象グループの設定・解除、表示日の変更でも同じ排他処理で世代を進める。取得失敗時は引き継いだキャッシュを表示し続ける。

Kotlinの描画処理は、現在の公開世代に対応するファイルだけを読み、キャッシュの生成元モード、対象グループ、世代が、現ビルドのモードと現在の選択状態にすべて一致する場合だけ旅行情報を表示する。フォールバック復旧も現在の公開世代に対応する最終更新時刻を参照する。従来形式のキャッシュはオンラインの第0世代として扱う。

通常の定期更新と、端末再起動後を含むKotlinのフォールバック定期・即時更新は、同じ保存値（Androidでは`FlutterSharedPreferences`の`flutter.resolved_app_mode`）からネットワーク制約を選ぶ。オンラインだけ接続済みネットワークを要求する。Gradleは`--dart-define`の指定をDartと同じ規則で解決し、`auto`と`online`をオンライン、`offline`をオフラインとして`BuildConfig`へ埋め込む。KotlinとDartは保存値をこの解決済みビルドモードと照合する。モード未保存・不明値・現ビルドとの不一致の場合は通常登録、フォールバック登録、バックグラウンド初期化を拒否し、アプリ起動によるモード保存後に登録する。アプリ起動時は、保存値が現ビルドの解決済みモードと異なる場合と不明値の場合に、旧モードの対象グループ・選択日・キャッシュ参照を消去してウィジェットを再描画してから、新しいモードを保存する。保存値がない従来版からの移行はオンライン由来として扱い、オフラインへ移行する場合だけ同様に消去する。このため、APK更新前のモードでFirebase・NTP・DBへアクセスせず、旧モードの旅行も表示しない。
