...
- [9. ユーザーとして、旅行の予定を管理したい。旅行中の予定を開始時刻順に整理したいから](#9-ユーザーとして旅行の予定を管理したい旅行中の予定を開始時刻順に整理したいから)
...
## 9. ユーザーとして、旅行の予定を管理したい。旅行中の予定を開始時刻順に整理したいから

```gherkin
Scenario: 旅行編集画面から予定編集画面へ遷移
  Given 旅行「沖縄旅行」が存在している
  And ユーザーは旅行「沖縄旅行」の編集画面を開いている
  When ユーザーが予定編集を開始する
  Then 予定編集画面に遷移する

Scenario: 予定を1件ずつリストに追加して登録
  Given ユーザーは旅行「沖縄旅行」の予定編集画面を開いている
  When ユーザーが予定「朝食」を開始時刻「2024-01-02 08:00」、終了時刻「2024-01-02 09:00」でリストに追加する
  And ユーザーが予定「首里城観光」を開始時刻「2024-01-02 10:00」、終了時刻「2024-01-02 12:00」でリストに追加する
  And ユーザーが保存する
  Then 旅行「沖縄旅行」の予定として「朝食」と「首里城観光」がplansに永続化される

Scenario: 予定の編集
  Given 旅行「沖縄旅行」に予定「朝食」が登録されている
  And ユーザーは旅行「沖縄旅行」の予定編集画面を開いている
  When ユーザーが予定「朝食」の開始時刻を「2024-01-02 08:30」に変更する
  And ユーザーが保存する
  Then 予定「朝食」の開始時刻が「2024-01-02 08:30」に更新される
  And 変更内容がplansに永続化される

Scenario: 予定を開始時刻順に自動で並び替える
  Given ユーザーは旅行「沖縄旅行」の予定編集画面を開いている
  And 予定「首里城観光」が開始時刻「2024-01-02 10:00」でリストに追加されている
  When ユーザーが予定「朝食」を開始時刻「2024-01-02 08:00」でリストに追加する
  Then 予定リストは「朝食」「首里城観光」の順に表示される
```
