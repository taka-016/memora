...
- [20. ユーザーとして、Androidウィジェットで直近の旅程を確認したい。アプリを開かずに当日の予定をすばやく確認したいから](#20-ユーザーとしてandroidウィジェットで直近の旅程を確認したいアプリを開かずに当日の予定をすばやく確認したいから)
...
## 20. ユーザーとして、Androidウィジェットで直近の旅程を確認したい。アプリを開かずに当日の予定をすばやく確認したいから

```gherkin
Scenario: Androidウィジェットの対象グループを設定する
  Given ユーザーが所属するグループ「家族」が存在している
  And ユーザーは設定画面を開いている
  When ユーザーがグループ「家族」をAndroidウィジェットの表示対象に設定する
  Then グループ「家族」がその端末のAndroidウィジェット表示対象として保存される

Scenario: Androidウィジェットに対象グループの直近の旅程を表示する
  Given Androidウィジェットの表示対象グループとしてグループ「家族」が設定されている
  And グループ「家族」に現在日付以降で開始日が最も近い旅行「沖縄旅行」と旅程項目「朝食」「首里城観光」が登録されている
  When ユーザーがAndroidのホーム画面でウィジェットを表示する
  Then グループ「家族」の直近の旅行「沖縄旅行」が表示される
  And 旅行「沖縄旅行」の旅程項目「朝食」「首里城観光」が開始日時順に表示される

Scenario: Androidウィジェットで別の旅行の旅程を確認する
  Given Androidウィジェットにグループ「家族」の旅行「沖縄旅行」が表示されている
  And グループ「家族」に開始日が前後する別の旅行「京都旅行」がキャッシュされている
  When ユーザーがAndroidウィジェットの左右ボタンを押す
  Then 旅行開始日の順序に従って表示中の旅行とは別の旅行に切り替わる
  And 切り替え後の旅行の旅程項目が開始日時順に表示される

Scenario: Androidウィジェットでキャッシュ範囲外の旅行へ移動する
  Given Androidウィジェットに表示中の旅行、過去1件、未来3件の旅行がキャッシュされている
  When ユーザーがキャッシュ範囲より前後の旅行へ移動しようとする
  Then 対象グループの旅行と旅程がDBから追加取得される
  And 追加取得した旅行に切り替わる

Scenario: Androidウィジェットを手動で更新する
  Given Androidウィジェットの表示対象グループとしてグループ「家族」が設定されている
  And グループ「家族」の旅程が前回更新時の内容でキャッシュされている
  When ユーザーがAndroidウィジェットの更新ボタンを押す
  Then グループ「家族」の最新の旅行と旅程が取得される
  And 取得した内容でAndroidウィジェットの表示とキャッシュが更新される

Scenario: Androidウィジェットの対象グループが未設定の場合
  Given Androidウィジェットの表示対象グループが設定されていない
  When ユーザーがAndroidのホーム画面でウィジェットを表示する
