...
Scenario: 年月テーブルに利用可能ポイントを常時表示する
  Given ユーザーがDVCポイント利用計算画面を開いている
  When システムが年月テーブルを表示する
  Then 1行目に年が表示される
  And 同一年の月は年セルが列結合されて表示される
  And 2行目に月が表示される
  And 3行目に各月の開始時点の利用可能ポイントが表示される
  And 4行目以降に各月の利用登録済みポイントが表示される

Scenario: 各月の開始時点の利用可能ポイントを表示する
...
Scenario: 利用可能ポイント表示を選択して内訳を表示する
...
Scenario: 利用登録済みポイントの明細を月ごとに表示する
  ...
  When ユーザーが4行目以降の2025年10月の使用ポイント表示を選択する
...
Scenario: 利用可能ポイント表示と同時に利用登録できる
  Given ユーザーがDVCポイント利用計算画面を開いている
  And 選択月の開始時点で利用可能ポイントが存在している
  When ユーザーが利用ポイント数とメモを入力して利用登録を実行する
  Then 利用登録が保存される
  And 選択月の利用可能ポイント表示は変更されない
  And 選択月の翌月以降の利用可能ポイント表示が再計算されて更新される
  And 選択月の使用ポイント明細および合計が新しい利用登録内容を反映して更新されて表示される
```
