...
Scenario: 年表上でのDVCポイント利用表示
  ...
  When ユーザーが所属グループの年表を表示する
  Then 年表のグループイベント行の下に「DVCポイント利用」行が表示される
  And 利用年月に合致する年のセルに「利用年月」「利用ポイント数」「メモ」が改行で表示される

Scenario: 年表上のDVCポイント利用編集導線の表示
  Given ユーザーが所属グループの年表を表示している
  When ユーザーが「DVCポイント利用」行を確認する
  Then 「DVCポイント計算」ボタンは表示されない
  And 「DVCポイント利用」タイトルの下に編集ボタンが表示される

Scenario: DVCポイント利用編集画面への遷移
  Given ユーザーが所属グループの年表を表示している
  When ユーザーが「DVCポイント利用」行の編集ボタンを選択する
  Then DVCポイント計算画面に遷移する
```
