...
- [11. ユーザーとして、旅行で訪問する場所を地図上で編集したい。場所を視覚的に把握したいから](#11-ユーザーとして旅行で訪問する場所を地図上で編集したい場所を視覚的に把握したいから)
- [12. ユーザーとして、旅行で訪問する場所間の移動経路を確認したい。旅行の計画を立てやすくしたいから](#12-ユーザーとして旅行で訪問する場所間の移動経路を確認したい旅行の計画を立てやすくしたいから)
- [13. ユーザーとして、同じ場所の過去の記録を見たい。お気に入りの場所は何度も訪れることがあるから](#13-ユーザーとして同じ場所の過去の記録を見たいお気に入りの場所は何度も訪れることがあるから)
- [14. ユーザーとして、地図表示からも旅行情報を編集したい。場所から旅行を探すこともあるから](#14-ユーザーとして地図表示からも旅行情報を編集したい場所から旅行を探すこともあるから)
- [15. ユーザーとして、過去の旅行を検索したい。年表から探し出すのは大変だから](#15-ユーザーとして過去の旅行を検索したい年表から探し出すのは大変だから)
- [16. ユーザーとして、年表からイベントを管理したい。イベントは年別に分かれていると整理しやすいから](#16-ユーザーとして年表からイベントを管理したいイベントは年別に分かれていると整理しやすいから)
...
## 11. ユーザーとして、旅行で訪問する場所を地図上で編集したい。場所を視覚的に把握したいから

```gherkin
Scenario: 旅行の訪問場所編集を開く
  Given ユーザーがログイン済みで旅行「沖縄旅行」が存在している
  When ユーザーが旅行「沖縄旅行」の編集画面にアクセスする
  And 訪問場所の編集ボタンをタップする
  Then 地図画面が表示される

Scenario: 旅行に訪問場所を追加（検索）
  Given ユーザーがログイン済みで旅行「沖縄旅行」が存在している
  And ユーザーが旅行「沖縄旅行」の編集画面で訪問場所編集を実行し、マップが表示されている
...
```gherkin
Scenario: 訪問場所間の経路検索を開く
  Given ユーザーがログイン済みで旅行「沖縄旅行」が存在している
  And 訪問場所に「那覇空港」「首里城」「美ら海水族館」が追加されている
  When ユーザーが旅行「沖縄旅行」の編集画面にアクセスする
  And 訪問場所の経路検索ボタンをタップする
  Then 経路検索画面が表示される

Scenario: 経路検索
  Given ユーザーがログイン済みで旅行「沖縄旅行」が存在している
  And 訪問場所に「那覇空港」「首里城」「美ら海水族館」が追加されている
  And ユーザーが旅行「沖縄旅行」の編集画面で経路検索を実行している
  When 「那覇空港」、「首里城」、「美ら海水族館」の順に並んでいる
  And 経路検索ボタンをタップする
  Then 各訪問場所間の経路情報が表示される
  And マップ上に各訪問場所間の経路が線で表示される

Scenario: 経路検索結果の保存
  Given ユーザーがログイン済みで旅行「沖縄旅行」が存在している
  And 訪問場所に「那覇空港」「首里城」「美ら海水族館」が追加されている
  And ユーザーが旅行「沖縄旅行」の編集画面で経路検索を実行している
  When 各訪問場所間の経路情報が表示されている
  And 保存ボタンをタップする
  Then 経路情報が旅行「沖縄旅行」に保存される
```

## 13. ユーザーとして、同じ場所の過去の記録を見たい。お気に入りの場所は何度も訪れることがあるから

```gherkin
Scenario: 同じ場所の訪問記録の一覧表示
  Given ユーザーがログイン済みで「首里城」の位置がユーザーが所属するグループの旅行の「沖縄旅行2023」「沖縄旅行2024」の訪問場所として登録されている
  When ユーザーがマップで「首里城」のピンをタップする
  Then 「首里城」を訪問した過去の旅行があることが表示される
  And 過去の旅行一覧を開くと「沖縄旅行2023」「沖縄旅行2024」が表示される
  And 各旅行名をタップするとその旅行に遷移する

```

## 14. ユーザーとして、地図表示からも旅行情報を編集したい。場所から旅行を探すこともあるから
...
## 15. ユーザーとして、過去の旅行を検索したい。年表から探し出すのは大変だから
...
## 16. ユーザーとして、年表からイベントを管理したい。イベントは年別に分かれていると整理しやすいから
