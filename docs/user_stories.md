...
Scenario: ホーム画面でキャッシュ範囲外の旅行へ移動する
  Given ホーム画面表示用に表示中の旅行、過去1件、未来3件の旅行がキャッシュされている
  When ユーザーがキャッシュ範囲より前後の旅行へ移動しようとする
  Then 対象グループの旅行と旅程がDBから追加取得される
  And 追加取得した旅行に切り替わる

Scenario: ホーム画面の旅程表示を手動で更新する
  Given ホーム画面の表示対象グループとしてグループ「家族」が設定されている
  And グループ「家族」の旅程が前回更新時の内容でキャッシュされている
  When ユーザーがホーム画面の更新ボタンを押す
  Then グループ「家族」の最新の旅行と旅程が取得される
  And 取得した内容でホーム画面の表示とキャッシュが更新される
