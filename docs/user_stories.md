...
Scenario: DVCポイント計算画面を表示する
  ...
  And 2行目に月が表示される
  And 3行目に各月の開始時点の利用可能ポイントが表示される
  And 4行目に各月の利用登録済みポイントが表示される

Scenario: 各月の開始時点の利用可能ポイントを確認する
  Given 選択中グループの契約ごとのポイント付与履歴と利用登録履歴が存在している
  When ユーザーが年月「2025-10」を選択する
  Then 2025年10月開始時点で利用可能な合算ポイントが表示される
  And 利用可能ポイントは「有効期限内の各契約の合算ポイント - 選択月の前月末までに利用登録済みポイント」で計算される
  And 選択月内の利用登録済みポイントは当月の利用可能ポイント計算に含まれない
...
Scenario: ポイントを利用登録する
  ...
  Then 利用登録が保存される
  And 利用月の利用可能ポイント表示は変更されない
  And 利用月の翌月以降の利用可能ポイント表示が再計算されて更新される
