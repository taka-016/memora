...
- [14. ユーザーとして、グループイベントメモを管理したい。年表セルのメモとして過去の出来事や未来の予定を残したいから](#14-ユーザーとしてグループイベントメモを管理したい年表セルのメモとして過去の出来事や未来の予定を残したいから)
...
Scenario: 年表上でのグループイベント表示
  Given ユーザーの所属グループの2024年セルにグループイベントメモ「運動会」が存在している
  When ユーザーが所属グループの年表を表示する
  Then 年表のグループイベント行の2024年セルに「運動会」が表示される

Scenario: グループイベント行の表示領域の調整
  Given ユーザーの所属グループの2024年セルに長いグループイベントメモが存在している
  When ユーザーが所属グループの年表を表示する
  And ユーザーがグループイベント行の表示領域を拡大する
  Then 表示しきれていなかったグループイベントメモの内容が見えるようになる

Scenario: 年表セルからのグループイベントメモ編集
  Given ユーザーが所属グループの年表を表示している
  When ユーザーがグループイベント行のセルを選択する
  Then フリー入力メモのダイアログが表示される
  And ユーザーはそのセルに対応するグループイベントメモを編集できる
...
## 14. ユーザーとして、グループイベントメモを管理したい。年表セルのメモとして過去の出来事や未来の予定を残したいから

```gherkin
Scenario: グループイベントメモの追加
  Given グループ「家族」の2024年セルにグループイベントメモが存在しない
  When ユーザーが年表のグループイベント行の2024年セルを選択する
  And フリー入力メモに「運動会」を入力して保存する
  Then グループ「家族」の2024年セルにグループイベントメモ「運動会」が登録される

Scenario: グループイベントメモの編集
  Given グループ「家族」の2024年セルにグループイベントメモ「運動会」が存在している
  When ユーザーが年表のグループイベント行の2024年セルを選択する
  And フリー入力メモを「太郎の運動会」に変更して保存する
  Then グループ「家族」の2024年セルのグループイベントメモが「太郎の運動会」に更新される

Scenario: グループイベントメモの削除
  Given グループ「家族」の2024年セルにグループイベントメモ「運動会」が存在している
  When ユーザーが年表のグループイベント行の2024年セルを選択する
  And フリー入力メモを空にして保存する
  Then グループ「家族」の2024年セルのグループイベントメモが削除される
```
