...
- 作成日時・更新日時・有効期限などの時刻情報は永続化・内部処理ともにUTCで保持し、表示時のみ端末のタイムゾーンへ変換する
  - 現行データ(createdAt、updatedAtが対象)をUTC基準へ移行するスクリプトを用意する
  - 作成日時・更新日時・有効期限などの内部比較はUTCで行い、端末のタイムゾーンは表示用途に限定する
