...
- Androidウィジェットの旅程時刻表示と矢印操作不具合を修正する
  - 旅程の開始終了時刻は開始時刻、-、終了時刻の三行で縦に並べ、中央揃えで表示する
  - 左右矢印ボタン押下時にエラーになっていないか確認し、表示日付を切り替えられるようにする
...
- Androidホーム画面ウィジェットで対象グループの直近の旅程を表示する
  ...
  - 旅行名は左右ボタンの中央付近に表示し、長い場合は末尾を省略表示する
  - 更新ボタンはウィジェット左上、左ボタンより上の位置に表示する
...
- Androidウィジェット用のキャッシュ保存先と保存形式を実装する
  - 対象グループID、表示中旅行ID、最終更新日時などの小さい値はhome_widget経由で端末ローカルに保存する
  - 旅行・旅程キャッシュ本体はhome_widget経由で共有できるJSONファイルとして保存する
  - JSONにはversion、groupId、selectedTripId、lastUpdatedAt、キャッシュ済み旅行一覧、各旅行の旅程項目一覧を含める
  - selectedTripIdは現在ウィジェットに表示している旅行IDとし、キャッシュ済み旅行一覧には左右移動用に表示中の旅行、過去1件、未来3件を保存する
  - Android側でDB構造を扱わなくて済むように、旅行期間ラベルや時刻ラベルなどを含む表示用データとして保存する
  - ログアウト時や対象グループ変更時に古いキャッシュを削除する

## グループ年表画面

- 年表画面を開く際、所属グループが1件のみならグループ一覧を表示せず、そのグループの年表画面を直接表示する
  - 対象導線は `lib/presentation/app/top_page.dart` の「グループ年表」を開く入口のみに限定し、年表表示中の戻る操作や他画面からの復帰時には一覧スキップを再実行しない
  - グループ件数の判定は `lib/presentation/notifiers/group_timeline_navigation_notifier.dart` 起点の年表導線専用ロジックで行い、`lib/presentation/shared/group_selection/group_selection_list.dart` には自動遷移責務を持ち込まない
  - 判定結果が0件なら現状どおり「グループがありません」を表示し、2件以上なら現状どおりグループ一覧を表示する
  - 判定結果が1件なら既存の `showGroupTimeline` と同じ遷移結果で対象グループの `Timeline` を構築し、戻る操作では一覧画面を確認できる既存挙動を維持する
  - 単一グループ判定の件数確認とグループ一覧描画でDBアクセスを二重化せず、同一取得結果を入口判定と一覧描画で再利用する
...
- TopPage配下の疑似画面遷移で、Androidの戻る操作が画面上の戻る操作と同じ結果になるようにする
  ...
  - 旅行管理画面でAndroidの戻る操作を行った場合は、グループ年表画面へ戻し、既存の戻る操作と同じリフレッシュ処理を実行する
  - DVCポイント計算画面でAndroidの戻る操作を行った場合は、グループ年表画面へ戻し、既存の戻る操作と同じリフレッシュ処理を実行する
  - グループ一覧画面でのAndroidの戻る操作によるアプリ終了可否は、このタスクでは変更しない
  - 戻る遷移の実装は既存の`onBackPressed`相当の遷移処理を再利用し、画面ごとに戻り先を二重管理しない構成にする
...
- グループイベント行の下に「DVC」行を追加する
- dvc_point_usagesの利用年月に合致する年のセルへ、利用年月・利用ポイント数・メモを改行で表示する
  - メモがnullまたは空白のみの場合はメモ行を表示しない
- DVCポイント利用行の固定セルに編集導線を配置する
  - 「DVC」タイトルの横に編集ボタンを表示する
  - 固定セル全体のタップと編集ボタンタップのどちらでもDVCポイント計算画面へ遷移できるようにする
- 年表画面のDVC行をDvcCell化し、利用年月と利用ポイントを1行表示・メモ省略表示・高さ不足時の「…他◯件」表示に対応する（TripCell準拠、共通化含む）
...
- 旅程の編集時に場所を指定できるようにする
  ...
  - 旅行に紐づく既存locationsを表示する
  - locationピンをタップすると、そのlocationの`locationId`を旅程に設定する
  - その旅程に紐づくlocationのピンだけ赤色で表示する
  - 場所指定を解除した際、そのlocationを持つ旅程がなければlocation自体も削除
- 旅行編集画面の下部に旅行のlocationsマップを表示する
  ...
  - ピンをタップすると場所名と紐づく旅程名を表示し、紐づく旅程がない場合は「削除」ボタンからlocationを削除できる
  - マップの長押しで新しいlocationを設定する
  - 場所検索でlocationを設定することも可能
  - ここで追加したlocationは旅程に紐づけず、旅行にのみ紐づくlocationとして保存する
  - マップの初期表示位置は、①旅行に1件でもlocationが紐づいている場合は紐づく1件目の位置、②旅行にlocationが紐づいていない場合は、初期位置(または現在地)
...
- 旅行編集画面から旅程画面へ遷移できるようにする
  ...
  - 旅程ボタンはタスクボタンと横並びにし、左端から順に「旅程」「タスク」に見える配置にする
  - ボタン名は「旅程」「タスク」にする
- 旅程画面を作成する
  - 旅行編集モーダル内に旅程画面を表示する
  ...
  - 旅程項目の追加時は項目名のみ入力し、開始日時、終了日時、メモは編集画面で入力できるようにする
  - 旅程項目を選択すると編集画面を表示し、項目名、開始日時、終了日時、メモを変更できるようにする
  - 旅程項目の追加、編集、削除をできるようにする
  - 旅程画面の追加フォーム、一覧削除、編集ボトムシートはタスク画面のUI構成をベースにする
  ...
  - 旅程項目は開始日の年月日ごとに区切り線付きの見出しでまとめ、その単位で折りたたみ・展開できるようにする
  - 旅程一覧の項目は時刻を項目名の上に同じフォントサイズで表示し、開始日は見出しに任せて項目内では省略する。終了日は開始日と異なる場合のみ項目内に表示する
  - 旅程編集ボトムシートでは開始日時と終了日時をそれぞれクリアできるようにする
...
- PR396のlocationsマップの場所名編集と旅程場所変更UIを調整する
  ...
  - 灰色ピンのタップだけでは旅程の場所指定を変更せず、表示域内の「この場所を指定する」ボタンで変更する
...
- 上部にpinの位置情報からGoogle Places APIで場所名を取得して表示する
  ...
  - 場所名も更新処理時に保存する
- マップピンボトムシートの場所名再取得ボタンを廃止し、場所名を手動編集できるようにする
- 場所名の入力欄の左にあるピンマークのアイコンを削除する
...
- ユーザーストーリーのグループイベント仕様を、現状の「イベント名を年表に表示し、選択でイベント管理画面へ遷移する」前提から、次の仕様へ更新する
  ...
  - 年表セルのタップでフリー入力メモのダイアログを開き、メモを編集できるようにする
...
- DVCポイント計算画面レイアウト
  ...
  - 画面上部に、"契約管理"、"期間限定ポイント登録"のアイコンボタンを表示する
  - DVCポイント計算の表を表示する
- DVCポイント計算画面内で契約管理（ユーザーストーリー16）を実装する
  - DVCポイント計算画面内のボタンからダイアログで登録する（登録内容はdvc_point_contractsに準拠）
    - 契約名をリストカード形式で表示する
    - リスト最下部に＋ボタンで契約追加できる
    - 契約追加時やリストカードタップでカードが拡張し、契約情報の入力領域を表示する
    ダイアログ下部に他画面と同様に"キャンセル"と"更新"ボタンを配置
...
- DVCポイント計算画面（ユーザーストーリー17）を実装する
  ...
  - ユースイヤー基準で有効期限を判定して計算する（有効期限は付与月の1年前〜2年後）
  ...
  - 利用登録後に利用当月以降の利用可能ポイントを再計算し、利用登録済みポイント表示を更新する
...
- DVCポイント計算テーブルの表示密度を調整する
  - 列幅を従来の約1/3へ縮小する
  - 「年」行を廃止し、「年月」行で年/月の2段表示にする
  - 「さらに表示」を年月列の左右端の空列に配置する
- DVCポイント計算テーブルの初期表示範囲を未来側に限定する
  - 初期表示範囲を現在年月から5年後までに変更する（過去年月は初期表示しない）
- DVCポイント計算テーブルのヘッダ列を固定表示する
  - ヘッダ列（年月・利用可能ポイント・利用登録済ポイント）を横スクロール時に固定表示する
- DVCポイント計算テーブルのセル幅節約のため単位表示を省略する
  - 利用可能ポイントと利用ポイントのセルから`pt`表示を削除する
- DVCポイント計算画面の登録操作を縦3点メニュー化する
  - 契約登録と期間限定ポイント登録をアイコン表示から縦3点ボタンのポップアップ選択に変更する
- 利用ポイント内訳のリストから利用登録済ポイントを削除できるようにする
- 利用可能ポイント内訳と利用ポイント内訳のタイトルを年月で改行して2段表示にする
- 利用可能ポイントは、有効期限内で0ポイントになったものも0ptで表示する
- 利用可能ポイントの有効期限の上に有効期限と同フォントサイズでいつのユースイヤーのポイントなのかを表示する
- 利用可能ポイントがマイナスの場合、マイナス数値を赤字太字で表示する
- 利用ポイント登録後は画面全体を再読み込みせずに再計算し、横スクロール位置を維持する
- DVCポイント計算画面の各ダイアログ処理をそれぞれ別ソースに分離する
- DVCの契約管理ダイアログの各契約リストカードに契約削除ボタンを追加する
- 利用可能ポイントの内訳ダイアログ内で期間限定ポイントを削除可能にする
- 利用可能ポイントダイアログ内の有効期限表記をfrom〜to形式に変更する
- 利用可能ポイントダイアログ内で期間限定ポイントの有効期限とメモの表示順を逆にする
- 利用可能ポイント内訳ダイアログの有効期限表示から「有効期限:」の文言を削除する
- 期間限定ポイントおよび利用ポイントの削除時に確認ダイアログを表示する
- DVCポイント計算画面への導線をサイドメニューから年表画面へ移動する
  - ~~年表タイトルと年表の間のスペース右端に「DVCポイント計算」ボタンを配置する~~→仕様変更により廃止
  - 「DVC」タイトル横の編集ボタンからDVCポイント計算画面へ遷移する導線へ変更する
...
- 編集画面の変更状態をRiverpodで管理し、キャンセル時に確認ダイアログを表示する
  ...
  - メンバー編集画面に適用
  - 旅行編集画面に適用
- メニューから遷移する画面を表示中にAndroidの戻る操作を行った場合、グループ年表画面へ戻れるようにする
  - 対象は少なくとも地図表示、グループ管理、メンバー管理、設定、アカウント設定などTopPageのメニューから遷移する画面とする
  - ただし各画面内でさらに戻れる状態がある場合は、その画面内の戻る処理を優先する
  - 各画面内で戻る余地が無くなった時点で、TopPageのメニュー選択状態をグループ年表へ戻す
  - グループ年表画面内の戻る挙動や、グループ一覧画面でのアプリ終了可否はこのタスクでは変更しない
...
- Timelineの各行を差し替え可能な行定義インタフェースへ分離し、設定に応じて表示行と順番を組み替えられる構成にする
  - 行定義インタフェースには少なくとも固定列情報、年セル情報、セルタップ、背景色、初期高さを持たせ、`timeline.dart`内の行作成処理を置き換える
  - `Timeline`本体は「ヘッダー描画」「行リストの共通レイアウト」「横スクロール同期」「行リサイズUI」だけを担当し、使用する行は行クラスを注入することで追加できる構造にする
  - 行定義インタフェースから個別の行クラスを作成する(旅行行、イベント行、DVC行、メンバー行)。既存のセル用クラスはそれぞれの行クラスの中に統合する。
  - 行ごとのデータ取得、保存、詳細表示や管理画面への遷移は行クラス側へ集約し、`Timeline`本体は行のスクロール同期、行高さ、ヘッダー、表示設定だけを制御する。
  - 注入する行やその並び順の設定を今後データで管理する想定だが、今回に関しては注入する行と並び順はコードでベタ書きし、現行と見た目を変えない。
  - タイムライン画面の見た目や挙動は一切変更しない。
...
- DVCポイントの有効期限仕様を修正する（付与月の1年前〜2年後）
- DVCポイント計算画面の利用可能ポイント再計算起点を利用当月に変更する
