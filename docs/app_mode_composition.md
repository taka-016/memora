...
ウィジェットキャッシュは1つだけ公開し、生成元モード、対象グループ、選択日、旅行・旅程を保持する。Kotlinは生成元モードと対象グループが現在値に一致するキャッシュだけを表示する。選択日も公開済みJSONから取得し、別のSharedPreferences値との公開順序に依存しない。ファイルは一時ファイルへ書き終えてから固定パスへ置き換えるため、書き込み途中のJSONを公開しない。対象解除やモード変更ではキャッシュ参照と公開済みファイルを消去する。
