...
void main() {
  ...
  setUp(() async {
    ...
    await SqliteMemberRepository(db)
        .saveMember(const Member(id: '', displayName: '本人'));
    final rows = await db.customSelect('SELECT id FROM members').get();
    final memberId = rows.single.read<String>('id');
    groupId = await SqliteGroupRepository(db).saveGroup(
      Group(
        id: '',
        ownerId: memberId,
        name: '家族',
  ...
  test('グループの所属とメンバー情報を組み立てて取得する', () async {
    final group = await SqliteGroupQueryService(db)
        .getGroupWithMembersById(groupId);
    ...
    await SqliteGroupRepository(db).updateGroup(
      Group(id: groupId, ownerId: group.ownerId, name: '変更', members: []),
    );
    expect(
      (await SqliteGroupQueryService(db).getGroupWithMembersById(groupId))!
          .members,
      isEmpty,
    );
  });
  ...
  test('旅行を場所なしで保存し日時とタスクの並び順を復元して更新削除する', () async {
    ...
    final id = await repository.saveTripEntry(
      TripEntry(
        id: '',
        groupId: groupId,
        year: 2026,
    ...
    final trip = (await query.getTripEntryById(
      id,
      tasksOrderBy: [const OrderBy('orderIndex')],
    ))!;
    ...
    await repository.updateTripEntry(
      TripEntry(id: id, groupId: groupId, year: 2027),
    );
  ...
  test('場所付き入力を拒否して既存の旅行を維持する', () async {
    final repository = SqliteTripEntryRepository(db);
    final id = await repository.saveTripEntry(
      TripEntry(id: '', groupId: groupId, year: 2026),
    );
    await expectLater(
      repository.updateTripEntry(
        TripEntry(
          id: id,
          groupId: groupId,
          year: 2027,
    ...
    expect(
      (await SqliteTripEntryQueryService(db).getTripEntryById(id))!.year,
      2026,
    );
  });
  ...
  test('複数の保存途中で失敗した場合は全件ロールバックする', () async {
    await expectLater(
      SqliteWriteTransaction(db).run((scope) async {
        final repository = scope.repository<TripEntryRepository>();
        await repository.saveTripEntry(
          TripEntry(id: '', groupId: groupId, year: 2026),
        );
        await repository.saveTripEntry(
          TripEntry(id: '', groupId: 'missing', year: 2026),
        );
      }),
      throwsA(isA<Exception>()),
    );
    expect(
      await SqliteTripEntryQueryService(db).getTripEntriesByGroupId(groupId),
      isEmpty,
    );
  });
}
