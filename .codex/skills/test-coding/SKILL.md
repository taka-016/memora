---
name: test-coding
description: このリポジトリでユニットテストやインテグレーションテストを追加・修正するときだけでなく、TDDで実装・修正・リファクタリングを進めるコード変更ターン全般で使うテストスキル。テスト配置、命名、Mockito利用、TestException、Completerを使った非同期テスト制御に従ってRedから着手したいときに使う。
---
...
- TDDで進めるコード変更ターンでは、テストファイルを新規作成する場合だけでなく既存テストの追加修正や失敗テストの確認にもこのスキルを使う。
