...
- Flutter/SDK、Dartパッケージ、Androidビルド環境、互換性修正など、独立して検証・取り消せる作業ポイントごとにコミットし、ロールバックしやすい履歴を保つ。
...
各作業ポイントでは、次の作業へ進む前に差分を確認し、その時点で必要な検証を通してからコミットする。各コミットは単独でrevertできる一つの関心事に限定し、互換性上不可分な変更だけを同じコミットに含める。複数の作業ポイントを最後に一括コミットしない。
